...
{
    [CreateAssetMenu(fileName = "SellDrinksTask", menuName = "QuestConfigs/SellDrinksTaskConfig", order = 1)]
    public class SellDrinksTask : Task
    {
        [SerializeField] private int _minTargerValue;
        [SerializeField] private int _maxTargerValue;
        [SerializeField] private ItemType _itemTypeDrink;
        ...
        protected override void Initialization()
        {
            ...
            if (!_isChainTask)
            {
                ...
                _itemTypeDrink = GetItemType();
                PlayerPrefs.SetInt("SellItemDrinkTaskSaveItem",(int)_itemTypeDrink);
            }
        ...
        public override void LoadProgress(int currentValue, int targetAmount, bool isCompleted, bool isReceived)
        {
            ...
            if (!_isChainTask)
            {
                int savedItemType = PlayerPrefs.GetInt("SellItemDrinkTaskSaveItem", (int)ItemType.Coffee);
                _itemTypeDrink = (ItemType)savedItemType;
            }
        ...
        private void ChangeValue(Order order)
        {
            ...
            if (order.DrinkItemOrder == null || order.DrinkItemOrder != _itemTypeDrink)
            {
                Debug.Log("Напиток НЕВЕРНЫЙ или NULL ");
                return;
            }
